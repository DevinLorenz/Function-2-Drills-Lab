...
const totalEachOrder = orders.map(element => (element[`price`] * (element[`tax`] + 1 )))
console.log(totalEachOrder)
...
const sumOfPurchases = purchases.map(element => (element[`price`])).reduce((acc,curr) => (acc + curr))
